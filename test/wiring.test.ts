...
test("project Coral config registers local Eve-backed agent manifests", async () => {
  ...
  for (const agent of AGENTS) {
    ...
    assert.match(manifest, /format = "OpenAI"/);
    assert.match(manifest, /model = "deepseek-v4-pro"/);
    assert.doesNotMatch(manifest, /OPENROUTER_FALLBACK_MODEL/);
    assert.doesNotMatch(manifest, /OPENROUTER_API_KEY/);
...
test("Eve agent files define specialists, safe blackboard tools, and Exa MCP connection", async () => {
  ...
  assert.match(rootAgent, /openai\/deepseek-v4-pro/);
  ...
  for (const agent of AGENTS) {
    ...
    assert.match(agentConfig, /openai\/deepseek-v4-pro/);
